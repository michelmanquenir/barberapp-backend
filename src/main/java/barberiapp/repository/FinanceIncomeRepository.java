...
public interface FinanceIncomeRepository extends JpaRepository<FinanceIncome, Long> {
    ...
    @Query("SELECT SUM(i.amount) FROM FinanceIncome i WHERE i.user.id = :userId AND i.date BETWEEN :from AND :to")
    Double sumByUserIdAndDateBetween(@Param("userId") String userId,
                                    @Param("from") LocalDate from,
                                    @Param("to") LocalDate to);

    @Query("SELECT i.type, SUM(i.amount) FROM FinanceIncome i WHERE i.user.id = :userId AND i.date BETWEEN :from AND :to GROUP BY i.type")
